...
const CookiePolicy = () => {
    ...
    const { refetch: fetchPolicy } = useFetch(
        `${apiEndpoints.fetchPolicy}?type=privacy`,
        {
            onSuccess: (data) => {
                if (data.success) {
    ...
    return (
        <div className="min-h-screen bg-slate-50 py-12 px-4 md:px-0">
            <div className="max-w-3xl mx-auto">
                ...
                >
                    <h1 className="text-4xl font-black text-slate-900 mb-8 border-b pb-6">{capitalize(policyData?.policyHeading) || "Terms of Service"}</h1>
