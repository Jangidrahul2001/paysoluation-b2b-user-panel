...
export default function UpiPayout() {
...

    {
      ...
      cell: ({ row }) => (
        ...
        <div className="flex flex-col">
          <span className=" text-slate-800 ">{row.original.UpiId}</span>
        </div>
      )
    },
  ...
  const accountColumns = [
    ...
    { header: "UPI Id", center: true, accessorKey: "upiId", className: " text-slate-500" },
    {
      header: "Action",
      id: "actions",
      center: true,
  ...
  if (rejectRequest("upi-payout1", profile?.requestedService)) return (<RejectedRequest service="upi-payout" pipeline="upi-payout1" />)

  if (!checkAssignedService("upi-payout", "upi-payout1", profile?.assignedServices))
    return (<NoPermission service="upi-payout" pipeline="upi-payout1" />);

  return (
    <PageLayout
      title="UPI Payout"
      subtitle="Execute instant money transfers to any UPI ID with real-time processing"
      actions={
    ...
    >
      {isLoading ? (
        ...
        <div className="flex flex-col gap-8 text-left font-sans">
          ...
          <ConfirmationModal
            ...
            description="Are you sure you want to decouple this UPI Id account? This action is irrevocable."
      ...
      {isModalOpen && <AddAccountUpiPayout
        isOpen={isModalOpen}
